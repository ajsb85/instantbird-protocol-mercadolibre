...
Conversation.prototype = {
  ...
  sendMsg: function (aMsg) {
    if (this._disconnected) {
      this.writeMessage("ml", "This message could not be sent because the conversation is no longer active: " + aMsg, {system: true, error: true});
      return;
    }
...
Account.prototype = {
  connect: function() {
    ...
    setTimeout((function() {
      ...
      this._conv.writeMessage("ml", "You are now talking to /dev/null", {system: true});
    }).bind(this), 0);
  },
  _conv: null,
  disconnect: function(aSilent) {
    ...
    if (!aSilent)
      this._conv.writeMessage("ml", "You have disconnected.", {system: true});
    if (this._conv) {
...
function mlProtocol() { }
mlProtocol.prototype = {
  get normalizedName() "ml",
  get name() "MercadoLire",
  get iconBaseURI() "chrome://prpl-ml/skin/",
  options: {
...
mlProtocol.prototype.__proto__ = GenericProtocolPrototype;

const NSGetFactory = XPCOMUtils.generateNSGetFactory([mlProtocol]);
